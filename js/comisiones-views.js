/* ============================================================
   COMISIONES-VIEWS.JS
   Gestor de tabs/vistas del módulo Comisiones.
   Patrón idéntico a caja-views.js — funciona con cualquier
   estadoApp que siga la misma estructura de vistas.
   ============================================================ */
(function () {
  class GestorVistas {
    constructor() {
    ...
    construirTab(vista) {
      ...
      tab.innerHTML = `
        ...
        ${puedeCerrar ? `
          <button class="tab__cerrar" type="button" aria-label="Cerrar vista" data-cerrar-vista="${vista.id}">
          ...
          <button class="tab__guardar" type="button" aria-label="Guardar nombre" data-guardar-vista="${vista.id}">
    ...
    renderizar() {
      ...
      if (this.actualizarFlechas) requestAnimationFrame(() => this.actualizarFlechas());
    }
    ...
    actualizarContadorVistas() {
      ...
      document.querySelectorAll('[data-accion="agregar-vista"]').forEach(btn => {
        btn.textContent = `Agregar vista (${total}/50)`;
      });
    }
    ...
    clonarVista(id) {
      ...
      const nueva = this.agregarVista(
        ...
        `${vista.nombre} (copia)`,
        vista.filtro,
        { filtrosPill: vista.filtrosPill ? { ...vista.filtrosPill } : {}, fija: false }
      );
      if (!nueva) return null;
      this.activarVista(nueva.id);
      window.mostrarToast?.("✓ Vista clonada");
      requestAnimationFrame(() => this.iniciarEdicionInline(nueva.id, { esNueva: false }));
      return nueva;
    }
    ...
    crearVistaInline() {
      ...
      const nueva = this.agregarVista(id, "Nueva vista", () => true, { filtrosPill: {}, fija: false });
      if (!nueva) return null;
      this.activarVista(id);
      requestAnimationFrame(() => this.iniciarEdicionInline(id, { esNueva: true }));
      return nueva;
    }
    ...
    confirmarEdicionInline(id) {
      ...
      if (this.editandoEsNueva) window.mostrarToast?.("✓ Vista creada");
      this._resetEstadoEdicion();
    }

    cancelarEdicionInline(id) {
      const vista = this.obtenerVista(id);
      if (!vista) { this._resetEstadoEdicion(); return; }
      if (this.editandoEsNueva) {
        this._eliminarVistaSilencioso(id);
        this._resetEstadoEdicion();
        return;
    ...
    escucharEventos() {
      this.contenedor.addEventListener("click", (e) => {
        ...
        if (cerrar) { e.stopPropagation(); this.cerrarVista(cerrar.dataset.cerrarVista); return; }
        const menu = e.target.closest(".tab__menu");
        if (menu) return;
        const titulo = e.target.closest("[data-tab-titulo]");
        if (titulo?.isContentEditable) { e.stopPropagation(); return; }
      ...
      this.contenedor.addEventListener("keydown", (e) => {
        ...
        if (!id) return;
        if (e.key === "Enter") { e.preventDefault(); this.finalizarEdicionDOM(id); this.confirmarEdicionInline(id); }
        if (e.key === "Escape") { e.preventDefault(); this.finalizarEdicionDOM(id); this.cancelarEdicionInline(id); }
      });
      ...
      if (btnLeft && btnRight) {
        btnLeft.addEventListener("click",  () => { tabs.scrollBy({ left: -180, behavior: "smooth" }); requestAnimationFrame(this.actualizarFlechas); });
        btnRight.addEventListener("click", () => { tabs.scrollBy({ left: 180,  behavior: "smooth" }); requestAnimationFrame(this.actualizarFlechas); });
